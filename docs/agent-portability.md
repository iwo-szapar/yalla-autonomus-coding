...
| MCP-only hosts | `yalla-mcp/` | Prompt/tool server for hosts that can request MCP context but cannot run Yalla hooks. |
...
- `off` / `normal`: no injected compact rules for hosts with runtime mode support.
